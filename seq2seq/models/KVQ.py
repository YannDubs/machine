...
from seq2seq.util.helpers import MLP, ProbabilityConverter, renormalize_input_length, get_rnn_cell, GaussianNoise
...
class BaseKeyValueQuery(nn.Module):
    ...
    Args:
        ...
        is_contained_kv (bool, optional): whether or not to use different parts of the controller output as input for key
    ...
    def _compute_input_size(self):
        return max(self.min_input_size, self.output_size) if self.is_contained_kv else self.hidden_size
...
class BaseKeyQuery(BaseKeyValueQuery):
    ...
    Args:
        ...
        dropout (float, optional): dropout probability to apply on the input of the generator.
        kq_noise_sigma (float, optional): relative standard deviation of the noise to apply to the input of the generator.
        kq_noise_sigma_output (float, optional): relative standard deviation of the noise to apply to the output of the generator.
        kwargs:
    ...
    def __init__(self, hidden_size, max_len,
        ...
        self.noise = GaussianNoise(kq_noise_sigma)
        ...
        self.noise_output = GaussianNoise(kq_noise_sigma_output)
    ...
    def forward(self, controller_out, input_lengths, additional):
        ...
        Args:
            controller_out (torch.tensor): tensor of size (batch_size, input_length, hidden_size) containing
                the outputs of the controller (encoder or decoder for key and query respectively).
        ...
        input_generator = self.noise(input_generator)
...
class QueryGenerator(BaseKeyQuery):
    ...
    Args:
         hidden_size (int): size of the hidden activations of the controller.
         max_len (int): maximum length of the source sentence.
         key_generator (KeyGenerator, optional): if given, will use the key generator for the query generator as well.
         kwargs:
             Additional arguments for the `BaseKeyQuery` parent class.
    """
...
class ValueGenerator(BaseKeyValueQuery):
    ...
    Args:
        ...
        min_generator_hidden (int, optional): minimum number fof hidden neurons to use if using a MLP.
        is_highway (bool, optional): whether to use a highway between the embedding an the output.
        is_res (bool, optional): whether to use a residual connection between the embedding an the output.
        min_generator_hidden (int, optional): minimum number fof hidden neurons to use if using a MLP.
        is_mlps (bool, optional): whether to use MLPs for the generators instead of a linear layer.
    ...
    def __init__(self, hidden_size, embedding_size,
        ...
        if self.is_highway:
            carry_size = 1 if is_single_carry else self.output_size
            self.carrier = MLP(self.input_size, max(min_generator_hidden, carry_size), carry_size)
    ...
    def forward(self, encoder_out, embedded, additional):
        ...
        Args:
            encoder_out (torch.tensor): tensor of size (batch_size, input_length, hidden_size) containing
                the hidden activations of the encoder.
            embedded (torch.tensor): tensor of size (batch_size, input_length, embedding_size) containing
        ...
        if self.is_highway:
            ...
            carry_rates = self.probabilty_converter(carry_rates)
