...
<<<<<<< HEAD:seq2seq/models/TopKDecoder.py
=======

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

>>>>>>> upstream/master:machine/models/TopKDecoder.py
...
=======
        self.pos_index = (torch.tensor(range(batch_size), dtype=torch.long,
                                       device=device) * self.k).view(-1, 1)
>>>>>>> upstream/master:machine/models/TopKDecoder.py
        ...
        if encoder_hidden is None:
            ...
            if isinstance(encoder_hidden, tuple):
                hidden = tuple([_inflate(h, self.k, 1)
...
=======
        sequence_scores = torch.full([batch_size * self.k, 1],
                                     fill_value=-float('inf'), device=device)
        sequence_scores.index_fill_(0, torch.tensor(
            [i * self.k for i in range(0, batch_size)], dtype=torch.long, device=device), 0.0)
        sequence_scores = sequence_scores

        # Initialize the input vector
        input_var = torch.transpose(torch.tensor(
            [[self.SOS] * batch_size * self.k], dtype=torch.long, device=device), 0, 1)
>>>>>>> upstream/master:machine/models/TopKDecoder.py
        ...
        for _ in range(0, max_length):
            ...
            # If doing local backprop (e.g. supervised training), retain the
            # output layer
            ...
            scores, candidates = sequence_scores.view(
                batch_size, -1).topk(self.k, dim=1)
            ...
            predecessors = (candidates / self.V + self.pos_index.expand_as(candidates)
    ...
    def _backtrack(self, nw_output, nw_hidden, predecessors,
                   symbols, scores, b, hidden_size):
        ...
        if lstm:
            ...
            h_n = tuple([torch.zeros(state_size, device=device),
...
>>>>>>> upstream/master:machine/models/TopKDecoder.py
        ...
        output = [step.index_select(0, re_sorted_idx).view(b, self.k, -1)
                  for step in reversed(output)]
        p = [step.index_select(0, re_sorted_idx).view(
            b, self.k, -1) for step in reversed(p)]
        if lstm:
            h_t = [tuple([h.index_select(1, re_sorted_idx).view(-1, b, self.k, hidden_size)
                          for h in step]) for step in reversed(h_t)]
            h_n = tuple([h.index_select(1, re_sorted_idx.data).view(-1,
                                                                    b, self.k, hidden_size) for h in h_n])
        else:
            h_t = [step.index_select(1, re_sorted_idx).view(-1, b, self.k,
                                                            hidden_size) for step in reversed(h_t)]
            h_n = h_n.index_select(
                1, re_sorted_idx.data).view(-1, b, self.k, hidden_size)
        s = s.data
